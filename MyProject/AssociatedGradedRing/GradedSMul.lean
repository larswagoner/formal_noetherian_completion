...
#check mem_filtration_iff_mem_Im
...
lemma filtration_smul_coe_eval {m n : ℕ} {x : A} {y : M} (hx : x ∈ (CanonicalFiltration I).N m) (hy : y ∈ F.N n) :
    ↑(filtration_smul F ⟨x, hx⟩ ⟨y, hy⟩ : M) = x • y := rfl

lemma filtration_fsmul_zero {m : ℕ} (n : ℕ) (x : (CanonicalFiltration I).N m) :
    filtration_smul F x (0 : F.N n) = 0 :=
  LinearMap.map_zero ((filtration_smul F) x)

lemma filtration_zero_fsmul (m : ℕ) {n : ℕ} (x : F.N n) :
    filtration_smul F (0 : (CanonicalFiltration I).N m) x = 0 :=
  LinearMap.map_zero₂ (filtration_smul F) x

lemma filtration_add_fsmul {m n : ℕ} (x y : (CanonicalFiltration I).N m) (z : F.N n) :
    filtration_smul F (x + y) z = filtration_smul F x z + filtration_smul F y z :=
  LinearMap.map_add₂ (filtration_smul F) x y z

lemma filtration_fsmul_add {m n : ℕ} (x : (CanonicalFiltration I).N m) (y z : F.N n) :
    filtration_smul F x (y + z) = filtration_smul F x y + filtration_smul F x z :=
  LinearMap.map_add ((filtration_smul F) x) y z

lemma filtration_smul_fsmul {m n : ℕ} (a : A) (x : (CanonicalFiltration I).N m) (y : F.N n) :
    filtration_smul F (a • x) y = a • (filtration_smul F x y) :=
  LinearMap.map_smul₂ (filtration_smul F) a x y

lemma filtration_fsmul_smul {m n : ℕ} (a : A) (x : (CanonicalFiltration I).N m) (y : F.N n) :
    filtration_smul F x (a • y) = a • (filtration_smul F x y) :=
  LinearMap.CompatibleSMul.map_smul ((filtration_smul F) x) a y

lemma filtration_one_fsmul {n : ℕ} (x : F.N n) :
    filtration_smul F ⟨1, show 1 ∈ (CanonicalFiltration I).N 0 by simp⟩ x = ⟨↑x, by rw [zero_add]; exact x.2⟩ := by
  unfold filtration_smul
  simp


def graded_smul_left  (F : I.Filtration M) (m n : ℕ) :
    (CanonicalFiltration I).N m →ₗ[A] (GradedPiece F n) →ₗ[A] (GradedPiece F (m+n)) where
  toFun := fun x ↦ Submodule.mapQ _ _ (filtration_smul F x) (by
      intro y h
  ...
  map_add' := by
    intro x y
    ext ⟨t⟩
    show ⟦filtration_smul F (x + y) t⟧ₘ = _
    rw [filtration_add_fsmul]
    rfl
  map_smul' := by
    intro a x
    ext ⟨t⟩
    show ⟦filtration_smul F (a • x) t⟧ₘ = _
    rw [filtration_smul_fsmul]
    rfl

def graded_smul_hom (F : I.Filtration M) {m n : ℕ} :
    GradedRingPiece I m →ₗ[A] GradedPiece F n →ₗ[A] GradedPiece F (m+n) :=
  Submodule.liftQ _ (graded_smul_left F m n) (by
    intro x h
    ext ⟨y⟩
    show ⟦filtration_smul F x y⟧ₘ = 0
    rw [←GradedPiece_mk_zero_iff]
    convert filtration_smul_deg h y.2 using 2
    abel
  )

def graded_smul (F : I.Filtration M) {m n : ℕ} :
    GradedRingPiece I m → GradedPiece F n → GradedPiece F (m+n) :=
  fun x y ↦ graded_smul_hom F x y

lemma graded_smul_of_mk {m n : ℕ} (x : (CanonicalFiltration I).N m) (y : F.N n) :
    graded_smul F ⟦x⟧ₘ ⟦y⟧ₘ = ⟦filtration_smul F x y⟧ₘ := rfl
...
lemma graded_one_smul {n : ℕ} (x : F.N n) :
    graded_smul F ⟦⟨1, show 1 ∈ (CanonicalFiltration I).N 0 by simp⟩⟧ₘ ⟦x⟧ₘ =
